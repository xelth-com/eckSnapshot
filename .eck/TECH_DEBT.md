...
- [ ] `skeletonizer.js` `emptyBody()` with `preserveDocs: true` (depth 6): a function that already has a leading JSDoc gets a fully empty `{}` body with no `/* ... */` placeholder — the leading-comment nodes are reassigned to `body.innerComments` but Babel's generator considers them already printed and skips them. Pinned as current behavior in `test/snapshotBuilder.fixtures.test.js` ("skeletonizes JS at depth 6"); fix = clone the comment nodes (or emit a fresh placeholder) instead of reusing them (found 2026-07-07)
- [ ] (informational) secret redaction is applied by the caller (`createSnapshot.js`) on rendered output, NOT inside `snapshotBuilder.js`/`renderFileAtDepth` — any new consumer of the render path must redact itself (noted 2026-07-07)
...
## Resolved
- [x] Broaden vitest coverage (2026-07-07) — suite grown 18 → 44 tests / 3 files: fixture-tree tests for `discoverFiles` (ignore rules, binary sniff, ML opt-in) and `renderFileAtDepth` (truncation, skeleton depths 5/6, maxFileSize throw) in `test/snapshotBuilder.fixtures.test.js`; `extractMetadata` journal parsing (Metadata block, `<journal>` variants, malformed JSON fallback) in `test/eckProtocolParser.test.js`
